...
function showAlert(message) {
    ...
    const audio = new Audio('https://www.zapsplat.com/wp-content/uploads/2015/sound-effects-77317/zapsplat_cartoon_pop_bubble_etc_004_77427.mp3');
    audio.play();
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function validateForm2() {
        ...
        const edadInputTaller = document.getElementById('edad-taller').value.trim();
        const telefonoTaller = document.getElementById('telefono-taller').value.trim();
        const correoTaller = document.getElementById('correo-taller').value.trim();
        const textareaTaller = document.getElementById('textarea-taller').value.trim();
        const capacitacionCheckbox = document.getElementById('capacitacion').checked;
        ...
        const edadTaller = parseInt(document.getElementById('edad-taller').value);
        
        if (edadTaller < 18) {
            showAlert(`Lo lamento, eres menor de edad para ${articulo} ${tipo}.`);
            return;
        }
        
        if (edadTaller > 120) {
            showAlert(`Al parecer pasaste a un plano superior para ${articulo} ${tipo}.`);
            return;
        }
        if (!telefonoTaller) {
            showAlert(`Por favor, ingrese su número de teléfono para ${articulo} ${tipo}.`);
            return;
        }
        if (!correoTaller) {
            showAlert(`Por favor, ingrese su correo electrónico para ${articulo} ${tipo}.`);
            return;
        }
        if (!textareaTaller) {
            showAlert(`Por favor, escriba su consulta para ${articulo} ${tipo}.`);
            return;
        }    
    }    
});
